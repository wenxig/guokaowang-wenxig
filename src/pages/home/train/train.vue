...
import { useUserTemp,Data } from "@/store/userTemp.ts";
import pagination from "@/components/pagination.vue";
const userTemp:Data = useUserTemp();
...
_.each(userTemp.all, function (v, i:number) {
  ...
  choosePapers[i].ok = !(_.max(v.all as number[]) ?? 0) == choosePapers[i].fullcount;
});
  ...
  <div class="main">
    ...
    <listBox>
      ...
      <li class="pageChunk" v-if="allPage > 1">
        <pagination @addPage="onPage++" :allPage="allPage" @subtractPage="onPage--"> </pagination>
      </li>
    </listBox>
    <Teleport to="body">
